...
void UART_Init(uint8_t uartx, UART_Config_t *config) {
 ...
 // Enable Power for the Selected UART
   ...
   // Get Peripheral Clock Frequency (Assuming PCLK = SystemCoreClock / 4)
PCLK = SystemCoreClock / 4;  
Fdiv = PCLK / (16 * config->baudRate);  

UARTx->LCR = 0x83; // Enable DLAB (to access DLL & DLM)
UARTx->DLM = (Fdiv >> 8) & 0xFF;  // High byte of baud rate
UARTx->DLL = (Fdiv & 0xFF);       // Low byte of baud rate
